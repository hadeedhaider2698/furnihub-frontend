...
export default function ProductDetail() {
  ...
  const handleFollowToggle = async () => {
    ...
    if (!product || !product.vendor) {
      return toast.error('Vendor information is missing.');
    }
    try {
  ...
  if (isLoading) return <div className="fixed inset-0 z-[100] bg-[var(--surface-2)] flex items-center justify-center lg:bg-transparent"><motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1 }} className="w-10 h-10 border-4 border-[var(--primary)] border-t-transparent rounded-full" /></div>;
  if (!product) return <div className="fixed inset-0 z-[100] bg-[var(--surface-2)] flex items-center justify-center">Product not found</div>;
    ...
    >
      ...
      <div className="lg:sticky lg:top-24 lg:h-fit">
        <div className="w-full aspect-square bg-[var(--surface)] relative select-none cursor-pointer overflow-hidden lg:rounded-3xl lg:shadow-xl" onDoubleClick={handleDoubleTap}>
          ...
          <AnimatePresence>
            {showHeartAnim && (
              <motion.div initial={{ scale: 0, opacity: 0, x: '-50%', y: '-50%' }} animate={{ scale: 1.2, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }} className="absolute top-1/2 left-1/2 pointer-events-none z-20"><Heart size={120} className="fill-white text-white drop-shadow-2xl" strokeWidth={0} /></motion.div>
            )}
          </AnimatePresence>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar lg:overflow-visible lg:pt-0">
        <div className="p-4 lg:p-0">
          ...
          <div className="hidden lg:flex items-center justify-between mb-8">
             <div className="flex items-center space-x-3">
                <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-[var(--border)]">
                   <img src={product.vendor?.shopLogo || `https://api.dicebear.com/7.x/initials/svg?seed=${product.vendor?.shopName}`} className="w-full h-full object-cover" />
                </div>
                <div>
             ...
             <div className="flex space-x-4">
                ...
                <button onClick={handleShare} className="p-2 bg-white rounded-full shadow-sm hover:scale-110 transition"><Share2 size={24} /></button>
             </div>
          </div>
          ...
          <div className="lg:hidden flex items-center space-x-2 mb-6 p-3 bg-[var(--surface)] rounded-xl border border-[var(--border)]">
              <img src={product.vendor?.shopLogo} className="w-8 h-8 rounded-full border border-[var(--border)]" />
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-[var(--border)] mb-10">
             <div className="flex items-center justify-between mb-4">
                <h3 className="font-serif font-bold text-xl text-[var(--primary)]">Customer Reviews</h3>
                <div className="flex items-center text-[var(--warning)]"><Star className="fill-[var(--warning)]" size={16} /><span className="ml-1 font-bold text-[var(--primary)]">{product.rating || 0}</span></div>
             </div>
             <p className="text-sm text-[var(--text-secondary)] italic">"Perfect addition to my living room. The wood quality is visible and it feels very premium." - Verified Customer</p>
          </div>
      ...
      <AnimatePresence>
        {showMenu && (
          <>
            ...
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="fixed bottom-0 left-0 right-0 bg-white rounded-t-3xl z-[120] p-6 pb-all md:max-w-[600px] md:mx-auto">
