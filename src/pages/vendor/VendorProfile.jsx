...
export default function VendorProfile() {
  ...
  if (isLoadingVendor) return <div className="min-h-screen flex items-center justify-center bg-[var(--surface-2)]"><motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1 }} className="w-8 h-8 border-4 border-[var(--primary)] border-t-transparent rounded-full" /></div>;

  return (
    <div className="bg-[var(--surface)] min-h-screen pb-20 lg:max-w-[600px] lg:mx-auto lg:border-x border-[var(--border)] overflow-x-hidden relative">
      {/* Header */}
      ...
      {/* Hero Banner */}
      <div className="w-full h-32 lg:h-40 bg-[var(--surface-2)] relative">
        {vendor?.shopBanner ? (
          <img src={vendor.shopBanner} alt="Banner" className="w-full h-full object-cover" />
        ) : (
          <div className="w-full h-full bg-gradient-to-r from-[var(--surface-2)] to-[var(--border)] opacity-50" />
        )}
      </div>
      ...
      <div className="px-4 -mt-12 relative z-1">
        <div className="flex items-end justify-between mb-4">
          <div className="w-24 h-24 rounded-2xl border-4 border-[var(--surface)] overflow-hidden bg-[var(--surface-2)] shadow-sm">
            <img 
          ...
          <div className="flex-1 flex justify-around mb-2 px-4">
            ...
            <div className="text-center">
              <p className="font-bold text-lg text-[var(--primary)]">{vendor?.rating || 'New'}</p>
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-[var(--text-primary)] mt-1">{vendor?.description || 'Your boutique for premium furniture.'}</p>
          {vendor?.address?.city && (
        ...
        <div className="flex space-x-2">
           {isOwner ? (
               <Link to="/vendor/settings" className="flex-1 bg-[var(--surface-2)] text-[var(--primary)] font-bold py-2 rounded-lg text-center text-sm border border-[var(--border)] hover:bg-[var(--surface)] transition-all flex items-center justify-center">
                   <Edit size={16} className="mr-2" /> Edit Storefront
               </Link>
           ) : (
               <>
                <button onClick={handleToggleFollow} className={`flex-1 font-bold py-2 rounded-lg text-sm transition-all shadow-sm ${isFollowing ? 'bg-[var(--surface-2)] text-[var(--primary)] border border-[var(--border)]' : 'bg-[var(--primary)] text-white'}`}>
                    {isFollowing ? 'Following' : 'Follow'}
                </button>
                <button onClick={() => toast('Messages coming soon!')} className="flex-1 bg-[var(--surface-2)] text-[var(--primary)] font-bold py-2 rounded-lg text-sm border border-[var(--border)] flex items-center justify-center">
      ...
      <AnimatePresence>
        {showMenu && (
          <>
            ...
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="fixed bottom-0 left-0 right-0 bg-white rounded-t-3xl z-[80] p-6 pb-safe md:max-w-[600px] md:mx-auto">
              <div className="w-12 h-1.5 bg-gray-200 rounded-full mx-auto mb-6" />
              <div className="space-y-1">
                <button onClick={handleShare} className="w-full flex items-center space-x-4 py-4 font-bold text-gray-800"><Link2 size={24} /><span>Copy Profile Link</span></button>
                <button onClick={handleShare} className="w-full flex items-center space-x-4 py-4 font-bold text-gray-800"><Share2 size={24} /><span>Share Profile via...</span></button>
